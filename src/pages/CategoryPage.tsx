...
const CategoryPage: React.FC<CategoryPageProps> = ({ products, addToCart }) => {
  ...
  const filteredProducts = products.filter(product => {
    ...
    return (product.categories && product.categories.some(c => c.toLowerCase() === normId)) || 
           (product.tags && product.tags.some(t => t.toLowerCase() === normId)) ||
           (normId === 'boys' && product.categories?.includes('boy')) ||
  ...
  // Scroll to top and set dynamic SEO when category changes
  useEffect(() => {
    window.scrollTo(0, 0);
    
    // Dynamic SEO Metadata
    const seoTitle = `${categoryName} | Kids Clothing & Outfits - Zeero Kids`;
    ...
    metaDescription.setAttribute('content', `Shop comfortable and durable ${categoryName} at Zeero Kids. Explore our premium collection of boys' and girls' outfits, dresses, and sets.`);
      ...
      <div style={{
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(28px, 5vw, 44px)', fontWeight: 700, margin: '0 0 12px 0' }}>
            {categoryName} - Comfortable & Durable Outfits
          </h1>
          <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.8)', fontStyle: 'italic' }}>
            Showing {filteredProducts.length} premium {filteredProducts.length === 1 ? 'item' : 'items'} for kids & babies
          </p>
        </div>
      </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {Object.entries(CATEGORY_NAMES).map(([key, name]) => (
                  <Link 
                    ...
                    style={{
                      fontSize: '14px',
                      color: categoryId === key ? 'var(--primary)' : 'var(--text)',
                      fontWeight: categoryId === key ? 700 : 500,
                      padding: '8px 12px',
                      borderRadius: '6px',
                      background: categoryId === key ? 'var(--primary-light)' : 'transparent',
                      transition: '0.2s',
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    <i className="fas fa-chevron-right" style={{ fontSize: '10px', opacity: categoryId === key ? 0.8 : 0.3 }}></i>
                  </Link>
                ))}
              </div>
            </div>
