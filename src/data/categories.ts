...
// Default categories - used as instant fallback while Supabase loads
export const DEFAULT_CATEGORIES: Category[] = [
  ...
  { id: 'socks', name: 'Socks', parentId: 'accessories', showInSidebar: false, order: 2 },
...
// Map from Supabase snake_case to our camelCase interface
const mapRow = (row: Record<string, unknown>): Category => ({
  id: row.id as string,
...
export const loadCategories = async (): Promise<Category[]> => {
  try {
    ...
    if (error || !data || data.length === 0) {
      console.warn('Supabase categories fetch issue, using defaults:', error?.message);
      return DEFAULT_CATEGORIES;
    }

    return (data as Record<string, unknown>[]).map(mapRow);
  } catch (err) {
    console.error('Exception fetching categories, using defaults:', err);
    return DEFAULT_CATEGORIES;
  }
};
...
// Keep saveCategories as no-op for backwards compatibility
export const saveCategories = (_categories: Category[]): void => {
  // No-op: Supabase is now the source of truth
};
